...
for (k in 13:15){
  dftmp <- NULL
  for (q in 1:5){
    tmp <- read.csv(paste0("032822/ML_grid_",k,"_fcomplex_p",q,"_KMfit.csv.gz"), stringsAsFactors = FALSE)
    num <- length(unique(tmp$sim))
    tmp$sim <- seq(num*(q-1)+1, num*q, 1)
    tmp$grid <- k
    dftmp <- rbind(dftmp, tmp)
  }
  dfcen <- rbind(dfcen, dftmp)
}
df <- rbind(df, dfcen)

grid.df <- data.frame(grid = 1:27, grid[ ,c(5:dim(grid)[2])])
...
write.csv(df, "KMfit_full_results.csv", row.names = F)
...
np_tmp_long <- data.frame(rbind(as.matrix(np_tmp[,1:8]), as.matrix(np_tmp[,c(1:7, 10)])))
names(np_tmp_long) <- c(names(np_tmp)[1:7], "value")
np_tmp_long$Metric <- c(rep("MSE", dim(np_tmp)[1]), rep("Corr", dim(np_tmp)[1]))
np_tmp_long$value <- as.numeric(np_tmp_long$value)

# --- Unbalanced design
np_tmp_long_unbalanced <- np_tmp_long[np_tmp_long$f_b_i == "L_L" & (np_tmp_long$dgp == "unbalanced" | np_tmp_long$p_b_i == "25_1"), ]; dim(np_tmp_long_unbalanced)
...
np_tmp_long_unbalanced <- np_tmp_long[np_tmp_long$f_b_i == "NL_NL" & (np_tmp_long$dgp == "unbalanced" | np_tmp_long$p_b_i == "25_1"), ]; dim(np_tmp_long_unbalanced)
...
np_tmp_long_unbalanced <- np_tmp_long[np_tmp_long$f_b_i == "NL_L" & (np_tmp_long$dgp == "unbalanced" | np_tmp_long$p_b_i == "25_1"), ]; dim(np_tmp_long_unbalanced)
...
# --- Censoring rate
np_tmp_long_CR <- np_tmp_long[np_tmp_long$cenM == "indX" & (np_tmp_long$cen_scale != 4 | (np_tmp_long$cen_scale == 4 &
                              np_tmp_long$p_b_i == "1_1" & np_tmp_long$f_b_i == "L_L" & np_tmp_long$dgp == "fcomplex")), ]; dim(np_tmp_long_CR)
group.colors <- c("red", "blue", "green","orchid1", "orange")
np_tmp_long_lasso <- np_tmp_long_CR[np_tmp_long_CR$estimator %in% c("sl_lasso", "tl_lasso", "fl_lasso", "xl_lasso", "rl_lasso"), ]
p1 <- ggplot(np_tmp_long_lasso, aes(x = cen_scale, y = value)) +
  geom_point(aes(color = estimator), size=2) +
  scale_color_manual(values=group.colors)+
  theme_bw() +
  ylab("") +
...
p2 <- ggplot(np_tmp_long_grf, aes(x = cen_scale, y = value)) +
  geom_point(aes(color = estimator), size=2) +
  scale_color_manual(values=group.colors)+
  theme_bw() +
  ylab("") +
...
p3 <- ggplot(np_tmp_long_lasgrf, aes(x = cen_scale, y = value)) +
  geom_point(aes(color = estimator), size=2) +
  scale_color_manual(values=group.colors)+
  theme_bw() +
  ylab("") +
...
png("Performance_by_CR_indX_KM.png", width = 11, height = 10, units = 'in', res = 800)
plot_grid(p1, p2, p3, nrow = 2, align = "v")
dev.off()


np_tmp_long_CM <- np_tmp_long[np_tmp_long$cenM == "dX" | (np_tmp_long$cen_scale == 4 & np_tmp_long$p_b_i == "1_1" & np_tmp_long$f_b_i == "L_L" & np_tmp_long$dgp == "fcomplex"), ]; dim(np_tmp_long_CM)
group.colors <- c("red", "blue", "green","orchid1", "orange")
np_tmp_long_lasso <- np_tmp_long_CM[np_tmp_long_CM$estimator %in% c("sl_lasso", "tl_lasso", "fl_lasso", "xl_lasso", "rl_lasso"), ]
p1 <- ggplot(np_tmp_long_lasso, aes(x = cenM, y = value)) +
  geom_point(aes(color = estimator), size=2) +
  scale_color_manual(values=group.colors)+
  theme_bw() +
  ylab("") +
...
p2 <- ggplot(np_tmp_long_grf, aes(x = cenM, y = value)) +
  geom_point(aes(color = estimator), size=2) +
  scale_color_manual(values=group.colors)+
  theme_bw() +
  ylab("") +
...
p3 <- ggplot(np_tmp_long_lasgrf, aes(x = cenM, y = value)) +
  geom_point(aes(color = estimator), size=2) +
  scale_color_manual(values=group.colors)+
  theme_bw() +
  ylab("") +
...
png("Performance_by_CR_dX_KM.png", width = 11, height = 10, units = 'in', res = 800)
plot_grid(p1, p2, p3, nrow = 2, align = "v")
dev.off()
