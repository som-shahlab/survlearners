...
generate_tutorial_survival_data <- function(n, p, p.b = NULL, p.i = NULL, f.b = NULL, f.i = NULL,
                                            ...
                                            n.mc = 10000, t0 = NULL, Y.max = NULL, dgp = "fcomplex") {
  ...
  if (dgp == "fcomplex") {
    ...
    ap <- 25
    NLX <- data.frame(apply(X[ ,1:ap], 2, FUN = indcator))           # nonlinear/binary version of X
    ...
    if (f.b == "L" & f.i == "L") {
      ...
      } else if (p.b == ap & p.i == 1) {
        betah <- rep(beta / sqrt(p.b), p.b)
        cox.ft <- (numerator / exp(X[ ,1:ap] %*% betah + (-0.5 - gamma * X[ ,2]) * W))^2
      } else if (p.b == ap & p.i == ap) {
        betah <- rep(beta / sqrt(p.b), p.b); gammah <- rep(gamma / sqrt(p.i), p.i)
        cox.ft <- (numerator / exp(X[ ,1:ap] %*% betah + (-0.5 - X[ ,1:ap] %*% gammah) * W))^2
      }
      ...
      if (p.b == 1 & p.i == 1) {
        ...
        cox.ft <- (numerator / exp(NLXs %*% betah + (-0.5 - X[ ,1:ap] %*% gammah) * W))^2
      }
    ...
    if (f.b == "L" & f.i == "L") {
      if (p.b == 1 & p.i == 1) {
        for (i in 1:n) {
          ...
          mu0sp[i] <- mean(pmin(cox.ft0, Y.max) > t0)
          mu1sp[i] <-  mean(pmin(cox.ft1, Y.max) > t0)
          catesp[i] <- mu1sp[i] - mu0sp[i]
          }
      } else if (p.b == ap & p.i == 1) {
        for (i in 1:n) {
          cox.ft0 <- (numerator / as.vector(exp(t(X[i,1:ap]) %*% betah + (-0.5 - gamma * X[i,2]) * 0)))^2
          cox.ft1 <- (numerator / as.vector(exp(t(X[i,1:ap]) %*% betah + (-0.5 - gamma * X[i,2]) * 1)))^2
          mu0sp[i] <- mean(pmin(cox.ft0, Y.max) > t0)
          mu1sp[i] <-  mean(pmin(cox.ft1, Y.max) > t0)
          catesp[i] <- mu1sp[i] - mu0sp[i]
        }
        ...
        for (i in 1:n) {
          cox.ft0 <- (numerator / as.vector(exp(t(X[i,1:ap]) %*% betah + (-0.5 - t(X[i,1:ap]) %*% gammah) * 0)))^2
          cox.ft1 <- (numerator / as.vector(exp(t(X[i,1:ap]) %*% betah + (-0.5 - t(X[i,1:ap]) %*% gammah) * 1)))^2
          mu0sp[i] <- mean(pmin(cox.ft0, Y.max) > t0)
          mu1sp[i] <-  mean(pmin(cox.ft1, Y.max) > t0)
          catesp[i] <- mu1sp[i] - mu0sp[i]
        }
      ...
      if (p.b == 1 & p.i == 1) {
        for (i in 1:n) {
          ...
          for (i in 1:n) {
            cox.ft0 <- (numerator / as.vector(exp(t(NLXs[i,, drop = FALSE]) %*% betah + (-0.5 - gamma * X[i,2]) * 0)))^2
            cox.ft1 <- (numerator / as.vector(exp(t(NLXs[i,, drop = FALSE]) %*% betah + (-0.5 - gamma * X[i,2]) * 1)))^2
            mu0sp[i] <- mean(pmin(cox.ft0, Y.max) > t0)
            mu1sp[i] <-  mean(pmin(cox.ft1, Y.max) > t0)
            catesp[i] <- mu1sp[i] - mu0sp[i]
          }
          ...
          for (i in 1:n) {
            cox.ft0 <- (numerator / as.vector(exp(t(NLXs[i,, drop = FALSE]) %*% betah + (-0.5 - t(X[i,1:ap]) %*% gammah) * 0)))^2
            cox.ft1 <- (numerator / as.vector(exp(t(NLXs[i,, drop = FALSE]) %*% betah + (-0.5 - t(X[i,1:ap]) %*% gammah) * 1)))^2
            mu0sp[i] <- mean(pmin(cox.ft0, Y.max) > t0)
            mu1sp[i] <-  mean(pmin(cox.ft1, Y.max) > t0)
            catesp[i] <- mu1sp[i] - mu0sp[i]
          }
        ...
        if (p.b == 1 & p.i == 1) {
          ...
          for (i in 1:n) {
            cox.ft0 <- (numerator / as.vector(exp(t(NLXs[i,, drop = FALSE]) %*% betah + (-0.5 - gamma * as.numeric(X[i,2] > 0.5)) * 0)))^2
            cox.ft1 <- (numerator / as.vector(exp(t(NLXs[i,, drop = FALSE]) %*% betah + (-0.5 - gamma * as.numeric(X[i,2] > 0.5)) * 1)))^2
            mu0sp[i] <- mean(pmin(cox.ft0, Y.max) > t0)
            mu1sp[i] <-  mean(pmin(cox.ft1, Y.max) > t0)
            catesp[i] <- mu1sp[i] - mu0sp[i]
          }
          ...
          for (i in 1:n) {
            cox.ft0 <- (numerator / as.vector(exp(t(NLXs[i,, drop = FALSE]) %*% betah + (-0.5 - t(NLXs[i,, drop = FALSE]) %*% gammah) * 0)))^2
            cox.ft1 <- (numerator / as.vector(exp(t(NLXs[i,, drop = FALSE]) %*% betah + (-0.5 - t(NLXs[i,, drop = FALSE]) %*% gammah) * 1)))^2
            mu0sp[i] <- mean(pmin(cox.ft0, Y.max) > t0)
            mu1sp[i] <-  mean(pmin(cox.ft1, Y.max) > t0)
            catesp[i] <- mu1sp[i] - mu0sp[i]
          }
