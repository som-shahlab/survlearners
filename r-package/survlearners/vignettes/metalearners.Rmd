...
*Randomized controlled trial (RCT)*: The treatment assignment is randomized and such that $W_i$ is independent of $(X_i, T_i(1), T_i(0))$, i.e., $W_i \independent (X_i, T_i(1), T_i(0))$ and $\mathbb P[W_i=1] = e$ with known $0<e<1$.

*Noninformative censoring*: Censoring is independent of survival time conditional on treatment assignment and covariates, i.e., $C_i \independent T_i\,\mid\,X_i, W_i$.
...
*Unconfoundedness*: The potential survival times are independent of the treatment assignment $W_i$ conditionally on baseline covariates, that is, $W_i \independent (T_i(1), T_i(0)) | X_i$.
...
Generate the survival time using a proportional hazard (PH) model as a function of covariates $X_1$, $X_2$, and treatment $W$
```{R}
set.seed(123)
n <- 1000
p <- 5
...
So, $u_{(1)}(x)$ is trained using data only from treated subjects, while $u_{(1)}(x)$ is fitted using data only from control subjects. T-learner then applies both fitted models and estimates the CATE as the difference between the imputed responses of treated and untreated subjects, i.e.,
...
The "missing data" or "right censoring" issue can be handled using Inverse Probability Censoring Weights (IPCW) [@kohler2002prediction; @Laan2003]. Specifically, We create a dataset with only \emph{complete} cases, which are defined as subjects who had an event before $t_0$ or finished the follow-up until $t_0$ without developing an event, i.e.,  $\mathcal{I}_{\text{comp}} = \{i \in \mathcal{I} : \Delta_i=1 \text{ or } U_i \geq t_0\}$. Next, we estimate the survival function of the censoring time as follows:
$$ S^C(u, x, w) = \mathbb{P}[C_i \geq u \mid X_i=x, W_i=w]$$
Based on $\widehat{S^C}(\cdot)$, we then re-weight every subject  $i \in \mathcal{I}_{\text{comp}}$ by $\what{K}_i$, an estimate of the inverse of the probability of not being censored, namely: 
$$\widehat{K}_i = 1\Big/\widehat{S^C}\left(\min\{U_i, t_0\}, X_i, W_i\right).$$
The IPCW adjustment removes censoring bias, but it can be inefficient and unstable when the censoring rate is high in a study and a majority of the censoring events happened before $t_0$. It also may be more sensitive to misspecification of the censoring model. In such cases, one can consider a doubly robust correction similar to the augmented inverse-propensity weighting estimator of @Robins1994. We do not dive into the details here and refer to interested readers to @tsiatis2006semiparametric.
...
If we hypothetically had access to the oracle scores $Y_i^{*}$ that satisfy $\mathbb E[Y_i^* \mid X_i=x] = \tau(x)\; \text{  or  }\; \mathbb E[Y_i^* \mid X_i=x]$ for the uncensored samples $i \in \mathcal{I}_{\text{comp}}$ and weights $\what{K}_i$, then we could estimate CATEs via weighted predictive modeling as $\hat{\tau}(\cdot) = \mathcal{M}(Y^{*} \sim X; \; \hat{K})$, where $\mathcal{M}$ symbolizes an arbitrary model. In the subsections below, we describe how each metalearner this challenge that---even in the absence of censoring---the oracle scores $Y_i^{*}$ are not available, due to the fundamental challenge of causal inference.
...
$$\hat{\tau}(x)= \mathcal{M}(Y^{*,M} \sim X;\; I_{\text{comp}}, \hat{K})$$
...
$$\hat{\tau}_{(1)}(\cdot) = \mathcal{M}(Y - \hat{\mu}_{(0)}(X) \sim X;\; I_{\text{comp}}^1, \hat{K}),$$
where $I_{\text{comp}}^1$ symbolizes the completed and treated subjects. Similarly, we can estimate $\hat{\tau}_{(0)}(\cdot) = \mathcal{M}(\hat{\mu}_{(0)}(X) - \sim X;\; I_{\text{comp}}^0, \hat{K})$. In the last stage, the X-learner combines the two CATE estimates as follows:
...
cate.test <- predict(surv.xl.grf.fit, X.test)$predictions
...
## R-learner
The *R-learner* @Nie2020 is a metalearner that builds upon a characterization of the CATE in terms of residualization of $W_i$ and $Y_i$ @Robinson1988, i.e., $W_i - e$ and $Y_i - m(x)$, where $m(x) = \mathbb E[Y_i \mid X_i=x]$. There are two ways to describe the R-learner -- through the loss-based and the weighted representations. 
...
where $\hat{m}(\cdot) = e \hat{\mu}_{(1)}(\cdot) + (1-e)\hat{\mu}_{(0)}(\cdot)$ is the outcome nuisance parameter estimated out-of-bag. The $\hat{\mu}_{(w)}(\cdot)$ as in the T-learner and $\hat{K}_i$ are the estimated censoring weights.
...
where $Y^{*, R} = \frac{Y_i-\hat{m}(X_i)}{W_i-e}-\widetilde{\tau}(X_i)\right$ is the constructed score by the R-learner. 

We point out several remarks about R-learner: 
1) R-learner is robust to a biased estimation of the outcome nuisance parameter, i.e., even if $\hat{m}(.)$ is not a good approximation to $m(\cdot)$, the R-learner may perform well @Nie2020. In fact, if $e = 0.5$ and we estimate $\what{m}(\cdot) \approx 0$ (even though $m(\cdot) \neq 0$), the R-learner predictions are very similar to the predictions of the M-learner. On the other hand, when $\what{m}(\cdot) \approx m(\cdot)$, then centering by $\what{m}(X_i)$ helps stabilize the estimation compared to the M-learner.
